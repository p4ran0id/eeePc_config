...
function API_sync_state()
{
	...
	jQuery.ajax({
		...
		success:CALLBACK_save_latest_sync_id,
		error:function(){
			track_event({
				category:'general',
				action:'TabSync',
...
/*
	...
	Params:
		xhr: XMLHttpRequest
*/
CALLBACK_save_latest_sync_id = function(latest_sid)
{
	// if(xhr.readyState == 4 && xhr.status === 200)
	// {
...
{		
	...
	sendRequest(method, url, headers, body, callback, null, timeoutFun);
}
	...
	if(xhr.readyState == 4 && xhr.status === 200)
	{
		...
		{	
			...
			for(var idx in updated_objs)
			{
				...
				if(updated_obj.payload && updated_obj.payload.tabs && updated_obj.payload.tabs.length > 0) {
                    // update tab sync time
                    if(CommInfo.fontNodes._baseTabTree.nodes.hasOwnProperty(updated_obj.cid)){
                        CommInfo.fontNodes._baseTabTree.nodes[updated_obj.cid].time.innerText = CommInfo.getCurTime();
                    }
			...
			else
			{
				...
				//save after_sid
				CommInfo.after_sid('tab', chunk_latest_sid);
...
*/
API_tab_sync_update = function(sync_id)
{
	var method = "POST";
	...
	sendRequest(method, url, headers, body_encode, callback, data.type, timeoutFun);
}
...
API_tab_sync_create = function()
{
	...
	var body_encode=urlencode(body);
	var callback = CALLBACK_save_tab_id;
	sendRequest(method, url, headers, body_encode, callback, null, timeoutFun);
}
...
{	
	if(xhr.readyState == 4 && xhr.status === 200)
	{
		...
		{	
			...
			// can't update lastsid because getTrunk may behind update/create 
			//set_normal_ico();
			showNotification('Sync Complete','Sync complete!');
			track_event({
				category:'general',
