...
function tab_sync_once(sid)
{
	...
	//print_msg(CommInfo.tab_sync_list())
	if(sid != null && CommInfo.is_login){
	}
}
...
*/
sync_caller = {
	bookmark: BookmarkSyncCtrl.sync	
};

function sync_control(types)
{
	var setting_json = CommInfo.get_setting();
	if(!types)
	{
	...
	{
		if(types[idx] in sync_caller && setting_json[types[idx]])
		{
			sync_caller[types[idx]].call();
			//if(types[idx] == 'bookmark')
			//{
		...
		{
			print_msg("Not support sync type:"+types[idx]);
		}
	}
