...
function setBelugaCode(user_id) {
	...
	setTimeout(function(){
		var appinfo ={"app_key":"69d1f93aa6cf4bf69e0bc893dd67de54","version":"2.0","chn":"chrome","uid":user_id, "target":"http://cn.belugaboost.com"};
...
function track_event(config, opt)
{
	...
	try {
		if(opt == null) {
			_gaq.push(['_trackEvent', 'chrome_cn', action, label, value]);
...
function set_sync_ico()
{
	...
	chrome.browserAction.setIcon({path:'images/sync.gif'});
}
...
function GetPushDevicesSorted()
{
	deviceList=CommInfo.device_list;
	onlineDevs=[];
	offlineDevs=[];
	devPushNum=localStorage['DolphinBrowserDevicePushNum'];
	if(devPushNum)
	{
		devPushNumJson = JSON.parse(devPushNum);	
	}
	else
	{
		devPushNumJson ={};
	...
	{
		if(deviceList[idx].state == 1)
		{
			onlineDevs.push(deviceList[idx]);
		}
		else
		{
			offlineDevs.push(deviceList[idx]);
		}
		if(devPushNumJson[deviceList[idx].did])
		{
			deviceList[idx].pushNum=devPushNumJson[deviceList[idx].did];		
		}
		else
		{
			deviceList[idx].pushNum=0;
		}
	}
	if(onlineDevs.length > 1)
	{
		onlineDevs.sort(function(a,b){return b.pushNum-a.pushNum;});
	}
	if(offlineDevs.length > 1)
	{
		offlineDevs.sort(function(a,b){return b.offTime-a.offTime;});
	}
	
	devices = onlineDevs.concat(offlineDevs);
	//print_msg(JSON.stringify(devices));
	return onlineDevs.concat(offlineDevs);
}
...
//chrome.tabs.create({url:chrome.extension.getURL('src.zip')}, function(tabs){});