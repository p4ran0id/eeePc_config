...
function onclick_img_handler(src, dev_id) {
	...
	track_event('TabPush','click','RightClickImg', 1);
}

function onclick_text_handler(text, dev_id) {
	if(text.length > 1000) {
		alert(extLocale["More than 1000 characters."]);
	}
	else {
		push(dev_id,{title:text,url:''},false, 3);
	}
	track_event('TabPush','click','RightClickText', 1);
}

function onclick_page_handler(title, url, dev_id) {
	push(dev_id,{title:title,url:url}, false, 1);
	track_event('TabPush','click','RightClickPage', 1);
}
...
function create_top_menu()
{
	CommInfo.context_menu_id = chrome.contextMenus.create({
		"title": extLocale["Send to Dolphin"], 
		"contexts":['page'],
		'documentUrlPatterns':["http://*/*","https://*/*"],
        "onclick": function(info,tab){
	    	if(CommInfo.device_list.length == 0) {
    			alert(extLocale["You haven't logged in with Dolphin Connect in your Dolphin Browser."]);
	    	}
	...
	CommInfo.context_image_menu_id = chrome.contextMenus.create({
		"title": extLocale['Send image to Dolphin'], 
		"contexts":['image'],
	    "onclick": function(info,tab){
	    	if(CommInfo.device_list.length == 0) {
	    			alert(extLocale["You haven't logged in with Dolphin Connect in your Dolphin Browser."]);
