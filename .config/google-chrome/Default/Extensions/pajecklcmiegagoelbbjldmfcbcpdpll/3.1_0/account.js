...
function thirdParty_login(type, win)
{
	...
	print_msg(API.signup + ThirdParty[type]+"?r=US&cid=" + get_clientid(false) +"&display=page&v=2&t=" + get_os_type()+"23");
		 ...
		 changeInfo.url.substr(0, API.loginresult[1].length) == API.loginresult[1]))
	{
		...
		if(ret_st == '0')
		{
			...
			else if(CommInfo.login_type == '11')
			{
				...
				CommInfo.login_typeName = extLocale['Sinaweibo'];
			}
			else if(CommInfo.login_type == '4') {
				CommInfo.login_typeName = extLocale['TencentQQ'];
			}
...
function login_init(tabId, first_login)
{
	if(first_login != null && first_login ==true) 
	{
		try {
			if(tabId) {
				chrome.tabs.executeScript(tabId, {code: GEN_INJECT_SCRIPT(INJECT_STATUS['LOGIN'], extLocale['Login Successful'])});
				...
				chrome.tabs.query({highlighted:true, windowId: chrome.windows.WINDOW_ID_CURRENT}, function(tabs){  
					...
					if((tab && (tab.url.indexOf("http://") != -1 || tab.url.indexOf("https://") != -1))){
						chrome.tabs.executeScript(tab.id, {code: GEN_INJECT_SCRIPT(INJECT_STATUS['LOGIN'], extLocale['Login Successful'])});
						chrome.tabs.executeScript(tab.id, {file: "cs_overlayer.js"});
					}
				});
			}
		}
	...
	// init common font node
	CommInfo.init_fontnode();
...
{	
    ...
    setTimeout(
    	function(){
			...
			delete localStorage['DolphinBrowserDevicePushNum'];
    	},
    	500
    );
    if(CommInfo && CommInfo.fontNodes){
    	CommInfo.fontNodes.clear();
    }
}
