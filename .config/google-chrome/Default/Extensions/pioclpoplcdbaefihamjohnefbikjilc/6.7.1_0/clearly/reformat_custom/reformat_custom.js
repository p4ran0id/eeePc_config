...
return false;} 
$RC.document=$RC.window.document;
...
var $=$RC.jQuery;$RC.$window=$($RC.window);$RC.$document=$($RC.document); $RC.detected=false; $RC.checkedSections=false;$RC.uncheckedSections=false;
$RC.debug=($RC.debug||false);$RC.debugRemembered={};if($RC.debug)
{
switch(true)
...
$RC.logOneline('-----------------------------------------');};
$RC.debugRemember=function(_k,_v)
...
$RC.composer__loop=function(_array,_callback)
...
{return $RC.escape_html(_text);};
$RC.sites={'amazon':{'sections_order':['top','images','bullets','book_description','product_description','dp_review','technical_details','reviews'],'sections_unchecked':false,'translations':{},'composer':function(_data,_loop,_div,_raw,_n2b,_esc,_br,_hr){return{'top':(function()
...
_br()+
_raw('<a href="'+_esc(_data.dp_review.more_url)+'" target="_blank">'+_esc(_data.dp_review.more_label)+'</a>')+'';return(_return?(_div(_data.headers.dp_review,'header',_br())+_return):'');})()};}
},'linked_in':{'sections_order':['top','summary','experience','skills','education','endorsements'],'sections_unchecked':false,'composer':function(_data,_loop,_div,_raw,_n2b,_esc,_br,_hr){return{'top':(function()
{var _return=''+
_div(_data.name,'header')+
...
_div(_item_endorsement.date_and_connection,'light')+'';});return _return_endorsements;})()+'';});return(_return_jobs?(_div(_data.headers.endorsements,'header',_br())+_return_jobs):'');})()};}
},'youtube':{'sections_order':['top','summary','comments'],'sections_unchecked':false,'composer':function(_data,_loop,_div,_raw,_n2b,_esc,_br,_hr){return{'top':(function()
{var _return=''+
...
_raw(_item.quote_html)+'';});return(_return?(_div(_data.headers.comments,'header',_br())+_return):'');})()};}
}
};

$RC.createFrame=function()
{
var _frame_id=$RC.settings.onCreateFrameUseThisId;
var _iframeElement=$RC.document.createElement('iframe'),_iframeBodyHTML=''+
...
_iframeBodyHTML+'</body>'+'</html>'+'';
_iframeElement.setAttribute('id',_frame_id);_iframeElement.setAttribute('frameBorder','0');_iframeElement.setAttribute('allowTransparency','true');_iframeElement.setAttribute('scrolling','auto');if($RC.settings.onCreateFrameUseThisURLAsTheLocation){_iframeElement.setAttribute('src',$RC.settings.onCreateFrameUseThisURLAsTheLocation);}
...
var _check1_interval=false;var _check1=function()
...
$RC.window.clearInterval(_check1_interval);};_check1_interval=$RC.window.setInterval(_check1,$RC.settings.onCreateFrameUseThisBaseTimer);
var _check2_interval=false;var _check2=function()
{ var _iframe=$RC.document.getElementById(_frame_id);if(_iframe){}else{return;} 
var _doc=(_iframe.contentDocument||_iframe.contentWindow.document);if(_doc){}else{return;} 
...
return _r;})();
var _html='';$RC.loopThroughSections(function(_section,_index)
...
$RC.composer__hr()+'';}); _html=_html.substr(0,(_html.length-$RC.composer__hr().length)); $RC.$iframeContentToDisplay.html(_html);
$RC.onlyCheckedSections();
$RC.$iframeContentToDisplay.find('div.checkbox').click(function()
...
$RC.onlyCheckedSections();});};
$RC.loopThroughSections=function(_callback)
...
_callback(_section,((_i===0)?0:((_i==_ii)?-1:_i)));}};
$RC.onlyCheckedSections=function()
...
{$RC.uncheckedSections.push(_section);}}); _htmlToSave=_htmlToSave.substr(0,(_htmlToSave.length-$RC.composer__hr().length)); $RC.$iframeContentToSave.html(_htmlToSave);};
$RC.getUncheckedSections=function()
...
$RC.sites[_site_id].sections_unchecked=_unchecked_sections;};
$RC.getContentToSaveNode=function()
{return $RC.$iframeContentToSave.get(0);};
$RC.getContentToSaveHTML=function()
